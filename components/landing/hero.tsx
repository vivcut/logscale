...
function FloatingPreview({ reduce }: { reduce: boolean }) {
  return (
    <motion.div
      ...
      transition
      className="overflow-hidden rounded-2xl border border-white/10 bg-black/60 text-left shadow-2xl backdrop-blur-xl"
    >
        ...
        {PREVIEW_ROWS.map((r) => (
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <span className="hidden font-mono text-[10px] uppercase tracking-wider text-white/40 sm:inline">
                {r.status}
              </span>
            </div>
            <span className="inline-flex items-center gap-1 rounded-md border border-white/10 px-2 py-1 font-mono text-xs text-white/80 tabular-nums">
