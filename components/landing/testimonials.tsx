    ...
    <section id="testimonials" className="relative mx-auto w-full max-w-6xl px-6 py-28">
      <Reveal className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="mt-3 text-balance text-3xl tracking-tight md:text-6xl">
          <span>Trusted by 600+ startups worldwide</span>
        </h2>
      ...
      <Stagger className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {TESTIMONIALS.map((t, index) => {
          // Determine the file extension dynamically (avatar6 uses .jpeg)
          const fileExtension = index === 5 ? "jpeg" : "png";
          
          return (
            <StaggerItem key={t.author}>
              <div className="flex h-full flex-col justify-between rounded-md bg-secondary p-6 transition-colors hover:border-primary/20">
                <blockquote className="text-lg italic leading-relaxed text-black">
                  {t.quote}
                </blockquote>
                <div className="mt-5 flex items-center gap-3">
                  <div className="flex size-10 items-center justify-center rounded-full overflow-hidden">
                    <Image
                      src={`/avatar${index + 1}.${fileExtension}`}
