  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      {questions.map((q, idx) => (
        <fieldset
          key={q.id}
          className="space-y-3 rounded-xl  border-2 border-border  bg-card p-5"
        >
          ...
          {q.type === "multiple_choice" ? (
            <div className="space-y-2">
              {q.options.map((opt, oi) => (
                <label
                  key={oi}
                  className="flex cursor-pointer items-center gap-2.5 rounded-xl  border-2 border-border  bg-background px-3.5 py-2.5 text-sm transition-colors hover:border-ring"
                >
                  <input
                    type="radio"
                    name={`q_${q.id}`}
