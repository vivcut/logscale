...
export default async function PlanPage({
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-10">
      ...
      {success ? (
        <div className="mb-6 rounded-xl border-2 border-primary/30 bg-primary/5 px-4 py-3 text-sm text-foreground">
          Payment received — your Startup plan is now active.
        </div>
      ) : null}
      {canceled ? (
        <div className="mb-6 rounded-xl border-2 border-border-2 bg-red-400/20 px-4 py-3 text-sm text-red-400 font-semibold">
          Checkout canceled. You haven&apos;t been charged.
        </div>
      ) : null}

      {hasPlan ? (
        <div className="flex items-center gap-4 rounded-xl border-2 border-primary/40 bg-primary/5 p-6">
