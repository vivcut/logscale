...
export function DashboardSidebar({
  ...
  const renderSidebarContent = (isMobileView = false) => {
    ...
    return (
      <>
        <div
          className={cn(
            "flex h-14 items-center  border-border-2 shrink-0",
        ...
        <div className={cn("px-3 pt-3", isCollapsedLayout && "px-2")}>
          <Link
            ...
            className={cn(
              "flex items-center rounded-xl !py-3 border-2 text-sm font-semibold transition-colors",
              isCollapsedLayout ? "justify-center px-2 py-2" : "gap-2 px-3 py-2",
              isStartup
                ? "bg-popover"
                : "border-border-2 bg-secondary/40 text-muted-foreground hover:text-foreground"
            )}
          >
            {isStartup ? (
        ...
        {/* Account */}
        <div className={cn("border-t-2 border-border-2 p-3", isCollapsedLayout && "px-2")}>
          <Link
  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-40 flex h-14 items-center justify-between border-border-2 bg-card px-6 md:hidden">
        ...
        <button
          ...
          className="flex size-11 items-center justify-center rounded-xl border-2 border-border-2 text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors"
        >
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 flex md:hidden">
          ...
          <aside className="relative flex w-64 max-w-xs flex-1 flex-col bg-card h-full border-r border-border-2 animate-in slide-in-from-left duration-200">
...
export function ThemeToggle({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <button
      ...
      title={`Theme: ${current.label}`}
      aria-label={`Theme: ${current.label} (click to change)`}
      className="flex w-full items-center justify-between rounded-xl px-3 py-3 text-sm font-semibold text-muted-foreground border-2 border-border-2 dark:border-popover transition-colors hover:bg-popover hover:text-foreground"
    >
