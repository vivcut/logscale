...
export function IncidentForm({ siteId }: { siteId: string }) {
  ...
  return (
    <form
      action={formAction}
      className="w-full rounded-lg border border-border bg-background/60 p-3"
    >
      ...
      <div className="mb-2 flex flex-wrap gap-1.5">
        {TAGS.map((t) => (
          <button
            ...
            onClick={() => setTag(t.id)}
            className={cn(
              "rounded-full border px-2 py-0.5 font-mono text-[10px] font-medium uppercase tracking-wide transition-opacity",
              t.cls,
